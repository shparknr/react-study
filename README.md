...
- package.json 에 devDependencies 에 설치한다는 뜻
- dependencies 에 설치하면 웹브라우저에 올라가는 용량이 너무 커짐.
...
- Node.js 프로젝트로 등록을 한다. (리액트와 상관없음)
...
- CRUD 테스트 (크러드)
  ...
  - Update ( PUT / PATCH )
  - Delete (DELETE)
- Postman (웹 API 테스트 시 일반적으로사용)
- Swagger (웹 API 테스트 시 별도로 BE에서 셋팅)
- Web Browser (웹 프로젝트 CORS 에러 발생함.)
  - package.json 에 proxy를 셋팅 해야한다.
  - "proxy": "주소" 작성해야 한다.
  - proxy : 내가 상대방 컴퓨터에서 작성하는 것 처럼 (속임)
